...
module controlUnit(op, funct, regwrited, memtoregd, memwrited, branchd, 
alucontrold, alusrcd, regdstd);
  ...
  assign alucontrold = (op==0 & funct==36) ? 0:
    ...
    (op==0 & funct==42) ? 7:
    3'hx, //default alucontrold value
    regwrited = (op==0 | op==8 | op==35) ? 1:0,
    memtoregd = (op==35) ? 1:
    (op==0 | op==8) ? 0: 1'bx, //default memtoregd value
    memwrited = (op==43) ? 1:0,
    branchd = (op==4) ? 1:0,
    alusrcd = (op==8 | op==35 | op==43) ? 1:0,
    regdstd = (op==0) ? 1: (op==8 | op==35) ? 0 :
    1'bx; //default regdstd value
endmodule